...
public class SoundtrackGeneratorShort
{
    const int SR = 44100;
    ...
    public static void Main()
    {
        ...
        // Phase 1 (0-8s): Ambient intro — Scene 1 directory
        PadLayer(buf, 0, totalSec, new double[] { 146.83, 174.61, 220.00 }, 0.10);
        Console.Write(" pad-low");
        PadLayer(buf, 0, totalSec, new double[] { 293.66, 349.23, 440.00 }, 0.05);
        Console.Write(" pad-mid");

        // Phase 2 (8-20s): Bass enters — Scene 1→2
        BassLine(buf, 8, totalSec, 0.18);
        Console.Write(" bass");

        // Phase 3 (20-32s): Percussion + resonant pads — Scene 2→3→4
        PadLayer(buf, 16, totalSec, new double[] { 146.83, 174.61, 220.00, 261.63 }, 0.05);
        Console.Write(" res-pad");
        Percussion(buf, 20, 73, 0.12);
        Console.Write(" drums");

        // Phase 4 (32-50s): Arpeggio + strings — Scene 4→5→6
        Arpeggio(buf, 32, 68, 0.05);
        Console.Write(" arp");
        PadLayer(buf, 42, totalSec, new double[] { 392.00, 466.16, 523.25, 587.33 }, 0.08);
        Console.Write(" strings");

        // Phase 5 (50-73s): High pads + brass + final — Scene 6→7 climax
        PadLayer(buf, 50, totalSec, new double[] { 587.33, 698.46, 880.00, 1046.50 }, 0.04);
        Console.Write(" highs");
        PadLayer(buf, 55, totalSec - 2, new double[] { 261.63, 329.63, 392.00, 440.00, 523.25 }, 0.05);
        Console.Write(" brass");
        PadLayer(buf, 62, totalSec - 2, new double[] { 293.66, 349.23, 440.00, 523.25 }, 0.07);
        ...
        for (int i = 0; i < buf.Length; i++)
        {
            ...
            if (t < 5.0) gain = t / 5.0;
            // Gentle rise (5-35s)
            else if (t < 35.0) gain = 1.0 + 0.3 * (t - 5.0) / 30.0;
            // Stronger rise (35-55s)
            else if (t < 55.0) gain = 1.3 + 0.4 * (t - 35.0) / 20.0;
            // Peak build (55-65s)
            else if (t < 65.0) gain = 1.7 + 0.3 * (t - 55.0) / 10.0;
            // Peak plateau (65-73s)
            else if (t < 73.0) gain = 2.0;
            // Fade out (73-75s)
            else gain = 2.0 * (1.0 - (t - 73.0) / 2.0);
